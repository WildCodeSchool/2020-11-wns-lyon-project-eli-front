...
export const createUserMutationGQL = gql`
  mutation create($firstName: String!, $lastName: String!, $email: String!, $password: String!, $role: String!) {
    createUser(
      values: { firstName: $firstName, lastName: $lastName, email: $email, password: $password, role: $role }
    ) {
      firstName
      lastName
      email
...
export const useCreateUserMutation = () => {
  ...
  // ts-ignore
  const createUser = async (values: UserInput): Promise<FetchResult<unknown>> => {
  ...
  return { createUser, mutationResults };
};
